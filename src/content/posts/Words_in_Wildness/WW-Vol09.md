...
- 新增「客邸」板块。为我一些朋友的内容腾出寄存的地方。我爱他们，也爱他们文字的质地。我想在中心化公司外为之多留一份副本。正在缓慢迁移中。

- 修改「一隅」板块。重写了很多内容，特别是 [🪶 Words](http://localhost:4321/about/#-words) 章节。我在这里介绍了「伏枥之间」的书写缘由、书写主题以及对 LLM 参与写作的态度。

  对了，即日起，本站承诺：**若无特殊声明，文字完全由人类书写；如有 AI 参与，将在标题处用「AI 使用披露标签」明确 AI 的参与比例与方式。**

- 修改「友邻」板块。比起单纯链接的交换，我会更在乎朋友们的文字和观点本身，**基于这些思想载体的交流，我想更接近*友链*这种形式作为「个体与个体」链接的初衷**。现在这个页面的「侧耳」板块（友邻的最新文章）被我放在更高优先级的位置，便利自己也便利访客通过彼此的文字触达彼此。

#### 微体验

- 增加昼夜主题切换的动画。当按钮激活时，以按钮为圆心，对应主题扩散将到整个页面（~~这个 feature 我自己可以玩很久！~~）。

  ![昼夜切换的新效果](WW-Vol09/image-20260527012421172.png)

- 当第一次打开本站，banner 动图会占据可观的加载时长。现在在 banner 加载未完成时，使用 LQIP[^1]与 slogan 占位，借鉴电影启幕前的画外音字幕。以期用完成度更高的设计来减少加载过程带来的「等待感」。

  ![首次访问，在头图加载过程中展示的内容](WW-Vol09/image-20260527012036630.png)

[^1]: 全称是 Low Quality Image Placeholder，即低质量图像占位符

- 正文图片资源也支持加载过程中使用 LQIP 图片占位。

- 正文图片资源增加 1 px 的边框，当图片与背景同色时也不会与内容混淆了。

- 「光影」板块中，采纳了金色河流的 [建议](https://leehenry.top/posts/words_in_wildness/ww-vol08/#b1c53aff64d34d7f91237dcd21afe9a5)，增加了随机展示的按钮。
...
- 「经纬」默认收起三级标题，当三级标题进入视口才展示，离开视口隐藏。

- RSS 订阅的原始页面并不重要，大家更多只想要拷贝链接，再粘贴到自己的阅读器中。现在点击 RSS 按钮并不会跳转，只会将链接复制到剪贴板中。
...
> 🤝 与我不谋而合的博客设计，又见：[Jant](https://www.owenyoung.com/jant) 的 *Hidden from Latest* 功能

- 优化了脚注的样式与展示形式，文中的脚注可以通过 hover 角标来预览内容。

- 增加正文的「折叠框」支持。使用「叠了一角的纸张」的样式来标识折叠的内容：hover 时纸张高亮、角落铺平；点击时折叠内容将完整显示。

  收起内容有 2 种方式：一种是再次点击折叠框的标题行，一种是点击尾部高亮的小三角，即可让内容收起并回滚到原始位置[^2]。试试看？

[^2]: 单纯因为自己的神秘强迫症：看完一定要折叠回去，不然怎么看都不对劲。
...
- 导航栏的 slash pages 将标题与引导语聚合，独立设计成统一的卡片。
...
不过目前遇到的问题是有些候选词比较诡异，一些专业词汇有所缺失（比如「信效度」）。还是需要调教磨合一段时间。
